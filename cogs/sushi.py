...
class Sushi(commands.Cog):
    ...
    def __init__(self, bot: discord.Bot) -> None:
        ...
        self.board_messages: dict[
            int, discord.Message
        ] = {}  # channel_id -> the live leaderboard message
    ...
    def build_embed(self, channel_id: int) -> discord.Embed:
        ...
        description = (
            ...
            if ranked
            else "nobody's eaten any sushi yet"
        )
        return discord.Embed(
            title="🍣 Sushi Leaderboard",
    ...
    async def repost_board(self, channel: discord.abc.Messageable) -> None:
        ...
        message = await channel.send(
            embed=self.build_embed(channel.id), view=SushiView(self)
        )
        self.board_messages[channel.id] = message

    @discord.slash_command(
        name="sushi", description="Show the sushi leaderboard", guild_ids=[GUILD_ID]
    )
