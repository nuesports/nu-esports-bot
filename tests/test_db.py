...
def test_get_db_conninfo_reads_secrets_yaml(monkeypatch):
    ...
    assert (
        db.get_db_conninfo()
        == "host=db port=5432 dbname=nu-esports-bot user=bot password=hunter2"
    )
...
async def test_perform_one_and_fetch_one_round_trip(clean_test_row):
    ...
    row = await db.fetch_one(
        "SELECT points FROM users WHERE discordid = %s;", (TEST_DISCORDID,)
    )
    assert row == (42,)
...
async def test_fetch_all_returns_every_matching_row(clean_test_row):
    await db.perform_one(
        "INSERT INTO users (discordid, points) VALUES (%s, %s);", (TEST_DISCORDID, 1)
    )
    rows = await db.fetch_all(
        "SELECT discordid, points FROM users WHERE discordid = %s;", (TEST_DISCORDID,)
    )
...
async def test_fetch_all_returns_empty_list_when_no_rows(clean_test_row):
    rows = await db.fetch_all(
        "SELECT * FROM users WHERE discordid = %s;", (TEST_DISCORDID,)
    )
    assert rows == []
