...
ACCOUNT_ROW = (
    ...
    None,
    "puuid-abc",
)  # index 4 is puuid, per ACCOUNT_COLUMNS ordering
...
def stub_rank_formatting(monkeypatch):
    monkeypatch.setattr(
        league, "compute_rank_value", lambda game, tier, division: (tier, division)
    )
    monkeypatch.setattr(
        league, "format_rank_label", lambda game, tier, division: f"{tier} {division}"
    )
...
@pytest.mark.asyncio
async def test_fetch_and_store_raises_on_unrecognized_rank(
    monkeypatch, skip_mains_seed
):
    _entries_client(
        monkeypatch, [{"queueType": "RANKED_SOLO_5x5", "tier": "GOLD", "rank": "V"}]
    )
...
@pytest.mark.asyncio
async def test_fetch_and_store_flat_tier_without_rank_field(
    fake_db, skip_mains_seed, monkeypatch
):
...
async def test_fetch_and_store_maps_roman_division_correctly(
    ...
    _entries_client(
        monkeypatch, [{"queueType": "RANKED_SOLO_5x5", "tier": "GOLD", "rank": "III"}]
    )
    client = league.LeagueClient()
    await client.fetch_and_store(123, ACCOUNT_ROW)
...
@pytest.mark.asyncio
async def test_fetch_and_store_skips_insert_when_no_solo_queue_entry(
    monkeypatch, fake_db, skip_mains_seed
):
    _entries_client(
        monkeypatch, [{"queueType": "RANKED_FLEX_SR", "tier": "GOLD", "rank": "III"}]
    )
    client = league.LeagueClient()
    await client.fetch_and_store(123, ACCOUNT_ROW)
